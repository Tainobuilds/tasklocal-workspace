...
export default function ReviewDialog({ bookingId, listingTitle, onClose, onSaved }: Props) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl my-auto">
        <header className="flex items-start justify-between gap-4 p-5 border-b border-slate-800">
          <div>
            <h2 className="font-semibold text-slate-100">How did it go?</h2>
        ...
        {saved ? (
          <div className="p-8 text-center space-y-3">
            ...
            <h3 className="font-semibold text-slate-100">Thanks for the review</h3>
            <button
              type="button"
              onClick={onClose}
          ...
          <form onSubmit={submit} className="p-5 space-y-4">
            <fieldset>
              <legend className="text-sm text-slate-300 font-medium mb-2">Rating</legend>
              <div className="flex items-center gap-1" onMouseLeave={() => setHovered(0)}>
                ...
                {rating > 0 && (
                  <span className="ml-2 text-sm text-slate-400 tabular-nums">{rating} of 5</span>
                )}
              </div>
            </fieldset>
            ...
            <label className="block">
              <span className="text-sm text-slate-300 font-medium">
                Feedback <span className="text-slate-500 font-normal">(optional)</span>
              </span>
              <textarea
                value={text}
